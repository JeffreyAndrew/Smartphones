...
class TestSmartphoneService(BaseTestCase):
    ...
    def test_single_smartphone(self):
        ...
        correctamente."""
        smartphone = add_smartphone('Prueba01', 'marca01', '100')
        with self.client:
            response = self.client.get(f'/smartphones/{smartphone.idSmartphone}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn('Prueba01', data['data']['name'])
            self.assertIn('marca01', data['data']['brand'])
    ...
    def test_all_smartphones(self):
        ...
        add_smartphone('nam1', 'brd1', '12')
        add_smartphone('nam2', 'brd2', '13')
        with self.client:
